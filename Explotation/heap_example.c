...
int main(int argc, char *argv[]) {
    ...
    strcpy(char_ptr, "This is memeory located on the heap.");
    printf("char_ptr (%p) --> '%s'\n", char_ptr, char_ptr);

    printf("\t[+] allocacting 12 bytes of memory on the heap for int_ptr\n");
    ...
    *int_ptr = 31337; // Put the value of 31337 where int_ptr is pointing
    ...
    if(char_ptr == NULL) {  // Error checking, in case malloc() fails 
       fprintf(stderr, "Error: could not allcoate heap memory.\n");
       exit(-1);
    }
